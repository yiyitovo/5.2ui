...
describe("Vault System - Deploy and Init", function () {
  ...
  // Test the collateral extraction function "withdraw ETH". Reasonable extraction (withdraw success)
  it("should allow user to withdraw ETH when collateral ratio is safe", async function () {
    const depositAmount = ethers.utils.parseEther("1"); // 1 ETH = $2000
    const borrowAmount = ethers.utils.parseUnits("1000", 18); // borrow 1000 DAI
    const withdrawAmount = ethers.utils.parseEther("0.1"); // Smaller withdrawals ensure that the mortgage ratio is sufficient
    ...
    const userCollateral = await vault.collateral(user.address);
    expect(userCollateral.toString()).to.equal(
      depositAmount.sub(withdrawAmount).toString()
    );
  });
  
  // Test the over-collateralizing function 'withdraw ETH, over-withdraw (withdraw fail）
  it("should revert if user tries to withdraw too much ETH and breaks collateral ratio", async function () {
    const depositAmount = ethers.utils.parseEther("1"); // 1 ETH = $2000
    const borrowAmount = ethers.utils.parseUnits("1200", 18); // borrow 1200 DAI
    const withdrawAmount = ethers.utils.parseEther("0.6"); // Try to withdraw 0.6 ETH (the collateral rate will be lower than 150%）
    ...
    await expect(
      ...
      ).to.be.rejectedWith("Withdrawal would breach collateral ratio");      
  });
